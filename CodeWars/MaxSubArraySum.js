var maxSequence = function(arr){
  ...
  for(let i = 0 ; i < arr.length ; i ++) {
    let sum = 0
    let accum = arr.slice(i).reduce((acc, cur) => {
      sum = (acc >= sum) ? acc : sum
      return acc + cur
    })
    
    sumArr[i] = (accum >= sum) ? accum : sum
  }
  return (Math.max.apply(null, sumArr) > 0) ? Math.max.apply(null, sumArr) : 0
}

// The `maximum sum subarray problem` consists in
// finding the maximum sum of a contiguous subsequence in
